    ...
    <div class="max-w-md mx-auto mt-10 p-6 border rounded-lg shadow-md space-y-6">
      ...
      <div>
        ...
        <p>
          ...
          {{ auth.user?.username || 'Chưa cập nhật' }}
        </p>
      </div>

      <!-- Form đổi mật khẩu -->
      <div class="mt-4">
        <x-form-button theme="primary" outline :disabled="disabled" @click="handleChangePassword">
          Đổi mật khẩu
        </x-form-button>
      </div>
      ...
      <!-- Đăng xuất -->
      <div class="mt-4">
        <x-form-button theme="primary" :disabled="disabled" @click="handleLogout">
          Đăng xuất
        </x-form-button>
      </div>
    </div>
