<template>
  <div class="flex flex-col w-full">
    ...
    <!-- Field -->
    <Field v-slot="{ field, errors, meta }" :name="name" :label="label" :rules="rules" as="div">
      <div ref="wrapperRef" class="relative w-full">
        <!-- Input / Selected display -->
        <div
          class="w-full rounded-2xl border px-4 py-2 transition-colors duration-200 outline-none bg-main border-primary hover:border-primary focus:ring-2 focus:ring-primary disabled:bg-gray-100 disabled:cursor-not-allowed h-[60px] text-body font-medium font-robo flex items-center justify-between cursor-pointer"
        ...
        >
          ...
          <div
            v-if="multiple && filteredOptions.length"
    ...
    <!-- Description -->
    <p v-if="description" class="mt-1 text-sm text-gray-500 font-robo">
      {{ description }}
    </p>
  </div>
</template>
