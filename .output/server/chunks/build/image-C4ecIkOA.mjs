...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const onLoad = () => {
      var _a;
      const el = ((_a = imgRef.value) == null ? void 0 : _a.$el) || imgRef.value;
      if (!el) return;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<!--[--><div class="w-full bg-gray-100 overflow-hidden" style="${ssrRenderStyle({ borderRadius: unref(radiusStyle) })}"><div class="w-full" style="${ssrRenderStyle({ aspectRatio: unref(width) / unref(height) })}">`);
      if (unref(cUrl)) {
        _push(ssrRenderComponent(unref(NuxtImg), {
          ...
          src: unref(cUrl),
          alt: __props.alt,
          class: ["w-full h-full object-cover object-center rounded-[inherit] transition-transform duration-300", { "cursor-pointer": unref(clickTo) }],
          width: unref(width),
          height: unref(height),
        ...
        _push(`<div class="absolute inset-0 flex items-center justify-center bg-gray-200 text-gray-400 text-sm rounded-[inherit]"> No Image </div>`);
      }
      _push(`</div></div>`);
      if (__props.title) {
        _push(`<p class="text-center font-light text-[14px] italic leading-[25px] pt-[10px]">${ssrInterpolate(__props.title)}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<!--]-->`);
...
//# sourceMappingURL=image-C4ecIkOA.mjs.map
